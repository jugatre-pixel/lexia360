...
# CONFIG
# ============================================================
APP_VERSION = "lexia360-v15-stable-trash-pdf"
...
if os.path.exists("static"):
    # OJO: static/ está en la raíz del repo (al lado de app/)
...
def _autofix_schema():
    """
    Auto-migración MVP (sin Alembic) para evitar deploys rotos
    cuando añadimos columnas.
    """
    with engine.begin() as conn:
        # usuario.creado_en
        if not _col_exists(conn, "usuario", "creado_en"):
        ...
        # inmueble.activo
        if not _col_exists(conn, "inmueble", "activo"):
        ...
        if not _col_exists(conn, "rulerun", "fecha_analisis"):
...
def generar_pdf_informe(inmueble: dict) -> bytes:
    ...
    dur = str(res.get("duracion_minima_anios", "—"))
...
def status():
    with Session(engine, expire_on_commit=False) as session:
        users_count = session.exec(select(text("count(*)")).select_from(text("usuario"))).one()[0]
        zonas_count = session.exec(select(text("count(*)")).select_from(text("zonatensionada"))).one()[0]
        return {"status":"✅ OK","usuarios_registrados":int(users_count),"zonas_tensionadas":int(zonas_count),"version":APP_VERSION}
...
def check_zona(municipio: str, comunidad_autonoma: str, fecha: date | None = None):
    ...
    with Session(engine, expire_on_commit=False) as session:
        ...
        return {"municipio":municipio,"comunidad_autonoma":comunidad_autonoma,"fecha":str(f),
                "zona_tensionada":zona is not None,"fuente_oficial": zona.fuente_oficial if zona else None}
...
# IMPORTANTE: evita el 422 que estabas viendo.
# Si tienes /inmuebles/{id_inmueble} declarado antes que /inmuebles/trash,
# FastAPI puede intentar parsear "trash" como int. Por eso:
# 1) dejamos /inmuebles/trash aquí,
# 2) y en tu frontend usa SIEMPRE "/inmuebles/trash".
